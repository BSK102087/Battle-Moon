...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		while RACE_ALL&bcrac~=0 do
			local bcatt=1
			for biatt=0,7 do
				if Duel.IsPlayerCanSpecialSummonMonster(tp,id,0,0x21,0,0,4,bcrac,bcatt) then
					brac=brac+bcrac
					break
				end
				bcatt=bcatt*2
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0
		or not Duel.IsPlayerCanSpecialSummonMonster(tp,id,0,0x21,0,0,4,brac,batt) then return end
		c:AddMonsterAttribute(TYPE_NORMAL,batt,brac,0,0,0)
