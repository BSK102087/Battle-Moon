...
function s.initial_effect(c)
	...
	c:RegisterEffect(e3)
	--Cannot Attack this card (Diff column)
	local e4=Effect.CreateEffect(c)	
	e4:SetType(EFFECT_TYPE_SINGLE)
	e4:SetCode(EFFECT_CANNOT_BE_BATTLE_TARGET)
	e4:SetProperty(EFFECT_FLAG_SINGLE_RANGE)
	e4:SetRange(LOCATION_MZONE)
	e4:SetCondition(s.colcon)
	e4:SetValue(s.coltarget)
	c:RegisterEffect(e4)
	--Must Attack Card (Same Column)
...
function s.coltarget(e,c)
	return not c:IsImmuneToEffect(e) and not (e:GetHandler():GetColumnGroup():IsContains(c) or e:GetHandler()==c) and not c:IsAttribute(0x10) and c:GetControler()~=e:GetHandlerPlayer()
end
function s.lumntarget(e,c)
	return not c:IsAttribute(0x10) and (e:GetHandler():GetColumnGroup():IsContains(c) or e:GetHandler()==c)
end
...
function s.repfilter(c,tp)
	return c:IsFaceup() and c:IsControler(tp) and c:IsLocation(LOCATION_MZONE) and (c:IsSetCard(0x1f4) or c:IsAttribute(0x10))
