...
function s.spfilter(c,e,tp)
	return c:IsType(TYPE_MONSTER) and (c:IsLocation(LOCATION_GRAVE) or c:IsFaceup()) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP)
end
...
function s.retop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.ReturnToField(tc) then
		local g=Duel.GetMatchingGroup(s.ovfilter,tp,LOCATION_REMOVED,LOCATION_REMOVED,nil)
		if #g==0 then return end
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_XMATERIAL)
		local g=Duel.SelectMatchingCard(tp,s.ovfilter,tp,LOCATION_REMOVED,LOCATION_REMOVED,1,1,c,tp,e)
		Duel.Overlay(tc,g)	
	end
end
