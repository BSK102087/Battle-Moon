...
function s.rctg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.IsExistingMatchingCard(s.filter,tp,LOCATION_HAND,0,1,nil) 
		and Duel.GetMZoneCount(tp,nil,tp,LOCATION_REASON_TOFIELD,zone)>0 and Duel.IsPlayerCanSpecialSummonMonster(tp,6039,0,TYPES_TOKEN,1000,1500,nil,nil,nil)
...
function s.rcop(e,tp,eg,ep,ev,re,r,rp)
	...
	local dc=Duel.SelectMatchingCard(tp,s.filter,tp,LOCATION_HAND,0,1,1,nil):GetFirst()
		...
		Duel.ShuffleHand(tp)
		if dc:IsType(TYPE_MONSTER) and Duel.IsPlayerCanSpecialSummonMonster(tp,6039,0,TYPES_TOKEN,1000,1500,dc:GetLevel(),dc:GetRace(),dc:GetAttribute()) then
			Duel.BreakEffect()
