...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		local brac=0
		local bcrac=1
		while RACE_ALL&bcrac~=0 do
			local bcatt=1
			for biatt=0,7 do
				if Duel.IsPlayerCanSpecialSummonMonster(tp,id,0,0x21,0,0,2,bcrac,bcatt) then
					brac=brac+bcrac
					break
				end
				bcatt=bcatt*2
		...
		e:SetLabel(brac)
		return brac~=0 and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
	end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RACE)
	local bcrac=Duel.AnnounceRace(tp,1,e:GetLabel())
	local batt=0
	local bcatt=1
	for biatt=0,7 do
		if Duel.IsPlayerCanSpecialSummonMonster(tp,id,0,0x21,0,0,2,bcrac,bcatt) then
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local brac=e:GetLabel()
	local batt=Duel.GetChainInfo(0,CHAININFO_TARGET_PARAM)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0
		or not Duel.IsPlayerCanSpecialSummonMonster(tp,id,0,0x21,0,0,2,brac,batt) then return end
	c:AddMonsterAttribute(TYPE_NORMAL+TYPE_TRAP+TYPE_TUNER,batt,brac,0,0,0)
